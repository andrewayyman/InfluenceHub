...
public interface IAdminService
{
    ...
    Task<IReadOnlyList<ContactResponse>> GetContactMessagesAsync(bool? isReplied, string? search, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<AdminUserResponse>> GetUsersAsync(string? search, UserRole? role, bool? isActive, CancellationToken ct = default);
    Task<bool> DeleteUserAsync(Guid userId, CancellationToken ct = default);

    Task<IReadOnlyList<CampaignListResponse>> GetCampaignsAsync(CampaignStatus? status, string? search, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<ReportResponse>> GetReportsAsync(ReportStatus? status, string? search, CancellationToken ct = default);
